    ...
    <main className="import-page">
      ...
      <section className="import-shell">
        ...
        <section className="mapping-card">
          <div className="mapping-head"><span>Uploaded column</span><span>Detected samples</span><span>Use as</span><span>Confidence</span></div>
          {preview.profiles.map((profile) => {
            ...
            return <div className="mapping-row" key={profile.index}>
              ...
              <div className={`mapping-confidence ${level.toLowerCase()}`}><strong>{level}</strong><small>{choice?.method || "Unmapped"}</small></div>
            </div>;
          })}
        </section>
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {section === "methodology" && <section className="methodology">
          ...
          <div className="method-flow"><article><span>1</span><div><h3>Interpret locally</h3><p>Read CSV, TSV or XLSX sheets, find the header row, profile values, and use a free browser-based semantic model to suggest column meanings.</p></div></article><article><span>2</span><div><h3>Confirm and validate</h3><p>Require date and sales, ask the user to confirm uncertain mappings, reject invalid core rows, and never invent missing business fields.</p></div></article><article><span>3</span><div><h3>Adapt the dashboard</h3><p>Enable profit, units, basket, category, product, payment and promotion analytics only when the mapped data supports them.</p></div></article><article><span>4</span><div><h3>Run rolling backtests</h3><p>Test the requested 7-day or 30-day horizon across up to eight chronological unseen periods without random splitting or future-data leakage.</p></div></article><article><span>5</span><div><h3>Select and qualify</h3><p>Select the lowest-WAPE candidate, report MAE and bias, and use empirical historical errors for the displayed planning range.</p></div></article></div>
          ...
          <div className="disclosure"><strong>Data-use disclosure</strong><p>Uploaded sales rows are processed locally and are not retained after the tab closes. On first use, the browser downloads open-source model weights for semantic column matching; only the column interpretation runs through that local model.</p></div>
        </section>}
        <footer><span>RetailPulse AI · CIS 6000 Research Prototype</span><span>Predictions support decisions; they do not replace managerial judgement.</span></footer>
      </section>
    </main>
