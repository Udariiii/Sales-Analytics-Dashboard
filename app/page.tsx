      ...
      <section className="workspace">
        ...
        {section === "forecast" && forecast && <>
          ...
          <section className="dashboard-grid forecast-grid">
            ...
            <article className="panel accuracy-card"><div className="panel-head"><div><p>HOW RELIABLE IS IT?</p><h2>Forecast accuracy on past sales</h2></div></div><div className="accuracy-score"><strong>{pct(forecastAccuracy)}</strong><span>Typical difference from past sales: {pct(forecast.winner.wape)}</span></div><dl><div><dt>Chosen approach</dt><dd>{friendlyModelName(forecast.winner.name)}</dd></div><div><dt>Typical daily difference</dt><dd>±{money.format(forecast.winner.mae)}</dd></div></dl></article>
            <article className="panel"><div className="panel-head"><div><p>WHY THIS ESTIMATE?</p><h2>Past performance of each approach</h2></div></div><div className="model-compare">{comparedModels.map((model) => <div className={model.name === forecast.winner.name ? "winner" : ""} key={model.name}><span>{friendlyModelName(model.name)}</span><strong>{pct(model.wape)} difference</strong><i><b style={{ width: `${model.wape / maxComparedWape * 100}%` }} /></i></div>)}</div><small className="fine-print">RetailPulse tests several statistical forecasting models and selects the one that best fits your sales history.</small></article>
